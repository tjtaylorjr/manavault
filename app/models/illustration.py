...
class Illustration(db.Model):
    ...
    card_uuid = db.Column(db.Integer, db.ForeignKey('cards.uuid'), nullable = True, unique = True)
    ...
    def __repr__(self):
        return f'Illustrations({self.id}, {self.card_uuid}, {self.alternate_cardface_id}, {self.artist}, {self.small_image}, {self.normal_image}, {self.large_image}, {self.highres_png}, {self.art_crop})'
    def to_dict(self):
        return {
            "id": self.id,
            "card_uuid": self.card_uuid,
            "alternate_cardface_id": self.alternate_cardface_id,
            "artist": self.artist,
