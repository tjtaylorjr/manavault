...
import backgroundIMG from '../../assets/backgrounds/curious-homunculus.jpg';
...
const DeckEditor = (props) => {
  ...
  useEffect(() => {
    let mounted = true;
    if (location.state.data && mounted) {
      dispatch({ type: 'RESET DECKLIST', payload: initialState })
      setDeckName(location.state.data.deck_name);
  ...
  useEffect(() => {
    ...
    if (storedDeckData.deckList.length > 0 && mounted) {
      const bgMenuOptions = storedDeckData.deckList.map((el) => {
        if(el) {
          const {card} = el;
          const {illustration, name} = card;

          return { value: illustration.art_crop, label: name }
        }
      })
  ...
  useEffect(() => {
    ...
    if (saveFlag && mounted) {
      ...
      (async () => {
        ...
        const res = await fetch(`/api/decks/${deck_id}/edit`, {
          method: "PUT",
          headers: {
        ...
        (async () => {
          const res3 = await fetch(`/api/decks/${edited_deck_id}`)
    ...
    <>
      <div className="deckeditor">
        <div className="deckeditor__header">
          <div className="deckeditor__user-panel-background">
            <div className="deckeditor__user-panel-background-wrapper">
              <div className="deckeditor__user-panel-background-image" style={{ backgroundImage: `url(${backgroundIMG})` }}></div>
            </div>
            <div className="deckeditor__user-panel-gradient"></div>
          </div>
        ...
        <div className="deckeditor__body">
          <div className="deckeditor__display-buttons-wrapper">
            <button className="deckeditor__container-button" title="Deck Builder" onClick={showCardContainer} >
              <GiCardExchange />
            </button>
            <button className="deckeditor__curve-button" title="Curve Display" onClick={showCurve} >
              <RiBarChartFill />
            </button>
            {/* <button className="deckeditor__list-button" title="List Display" onClick={showList} >
                <BiListUl />
              </button>
              <button className="deckeditor__stack-button" title="Stack Display" onClick={showStack} >
                <CgStack />
              </button> */}
            <button className="deckeditor__save-button" title="Save" onClick={saveDeck} >
              <AiFillSave />
            </button>
            <button className="deckeditor__delete-build-button" title="Reset Deck" onClick={() => dispatch({ type: 'RESET_DECKLIST', payload: initialState })}>
              <BiReset />
            </button>
          </div>
          <div className="deckeditor__deck-container">
            {containerFlag && (
              <div className="deckeditor__build-view">
                <div className="deckeditor__search-panel">
                  <div className="deckeditor__search-options-container">
                    <form onSubmit={handleSearch} className="deckeditor__search-options-form">
                      <div className="deckeditor__search-options-form-top">
                        <div className="deckeditor__search-options-form-text-field">
                          <input type="search" results="5" ref={searchRef} onChange={(e) => setSearchInput(e.target.value)} placeholder="Find Cards"></input>
                          <button></button>
                        </div>
                      </div>
                      <div className="deckeditor__search-options-form-filter-buttons-container"></div>
                      <button className="deckeditor__search-options-form-submit-button">
                        <ImSearch />
                      </button>
                    </form>
                  </div>
                  <div className="deckeditor__search-results-wrapper">
                    {/* TODO - automated highlighting buttons upon selection, two of them need special handling due to nature of svgs */}
                    {/* <div className="deckeditor__search-results-filters">
                      <button>
                    ...
                    <div className="deckeditor__search-results-container">
                ...
                <div className="deckeditor__deck-card-container">
                  <form className="deckeditor__deck-card-container-form-fields-container">
                    <div className="deckeditor__form-partition-1">
                      <input
                    ...
                    <Select
                      className="deckeditor__form-partition-2" placeholder="Select deck image (optional)"
                    ...
                    />
                    {/* <div className="deckeditor__form-partition-3">
                      <input
                        type="text"
                    ...
                    <div className="deckeditor__form-partition-4">
                  ...
                  <DeckDnd storedDeckData={storedDeckData} dispatch={dispatch} mainDeck={mainDeck} sideboard={sideboard} dropData={dropData} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                </div>
                {/* <div className="deckeditor__container-view-sideboard-wrapper">
                  <div className="deckeditor__container-view-sideboard-header">Sideboard</div>
            ...
            {curveFlag && (
              <div className="deckeditor__curve-view">
                <div className="deckeditor__curve-view-special-cards">
                  <div
                    className="deckeditor__curve-view-special-cards-commander"
                    style={commanders.length > 0 ? { display: '' } : { display: "none" }}
                  >
                    <div className="deckeditor__commander-container-title">
                      <div className="deckeditor__commander-container-title-text">Commander</div>
                    </div>
                    <div className="deckeditor__commander-container">
                  ...
                  <div
                    className="deckeditor__curve-view-special-cards-companion"
                    style={sideSlotCompanion.length > 0 ? { display: '' } : { display: "none" }}
                  >
                    <div className="deckeditor__companion-container-title">
                      <div className="deckeditor__companion-container-title-text">Companion</div>
                    </div>
                    <div className="deckeditor__companion-container">{sideSlotCompanion.length > 0 && sideSlotCompanion.map((card, i) => (
                      <DeckCardObject
                        key={i} data={card}
                        // num={card.in_sideboard}
                        showImagePreview={hoverAction}
                ...
                <div className="deckeditor__curve-view-grid">
                  <div className="deckeditor__main-container1-title">
                    <div className="deckeditor__main-container1-title-text">{`Creatures & Planeswalkers (` + mainDeck.filter((card) =>
                      card.card.type.includes("Creature") || card.card.type.includes("Planeswalker")).reduce((total, el) => total + el.in_deck, 0) + `)`}</div>
                    <div className="deckeditor__main-container1-row-description">
                      <p>{"(CMC 0-1)"}</p>
                      <p>{"(CMC 2)"}</p>
                      <p>{"(CMC 3)"}</p>
                      <p>{"(CMC 4)"}</p>
                  ...
                  <div className="deckeditor__main-container1">
                    ...
                    <div className="deckeditor__creature-container2">
                      {creatures2.length > 0 && creatures2.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_deck} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__creature-container3">

                      {creatures3.length > 0 && creatures3.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_deck} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__creature-container4">
                      {creatures4.length > 0 && creatures4.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_deck} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__creature-container5">
                      {creatures5.length > 0 && creatures5.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_deck} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__creature-container6plus">
                      {creatures6Plus.length > 0 && creatures6Plus.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_deck} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                  </div>
                  <div className="deckeditor__land-container-title">
                    <div className="deckeditor__land-container-title-text">{`Lands (` + mainDeck.filter((card) =>
                  ...
                  <div className="deckeditor__main-container2-title">
                    <div className="deckeditor__main-container2-title-text">{`Spells & Artifacts (` + mainDeck.filter((card) =>
                    ...
                    <div className="deckeditor__main-container2-row-description">
                      <p>{"(CMC 0-1)"}</p>
                      <p>{"(CMC 2)"}</p>
                      <p>{"(CMC 3)"}</p>
                      <p>{"(CMC 4)"}</p>
                  ...
                  <div className="deckeditor__main-container2">
                    ...
                    <div className="deckeditor__spell-container2">
                      {spells2.length > 0 && spells2.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_deck} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__spell-container3">
                      {spells3.length > 0 && spells3.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_deck} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__spell-container4">
                      {spells4.length > 0 && spells4.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_deck} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__spell-container5">
                      {spells5.length > 0 && spells5.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_deck} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__spell-container6plus">
                      {spells6Plus.length > 0 && spells6Plus.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_deck} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                  </div>
                  <div className="deckeditor__side-container-title">
                    <div className="deckeditor__side-container-title-text">{`Sideboard (` + sideboard.reduce((total, el) => total + el.in_sideboard, 0) + `)`}</div>
                    <div className="deckeditor__side-container-row-description">
                      <p>{"(CMC 0-1)"}</p>
                      <p>{"(CMC 2)"}</p>
                      <p>{"(CMC 3)"}</p>
                      <p>{"(CMC 4)"}</p>
                  ...
                  <div className="deckeditor__side-container">
                    <div className="deckeditor__sideslot01-container">
                    ...
                    <div className="deckeditor__sideslot2-container">
                      {sideSlot2.length > 0 && sideSlot2.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_sideboard} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__sideslot3-container">
                      {sideSlot3.length > 0 && sideSlot3.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_sideboard} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__sideslot4-container">
                      {sideSlot4.length > 0 && sideSlot4.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_sideboard} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__sideslot5-container">
                      {sideSlot5.length > 0 && sideSlot5.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_sideboard} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__sideslot6plus-container">
                      {sideSlot6Plus.length > 0 && sideSlot6Plus.map((card, i) => (
                        <DeckCardObject key={i} data={card} num={card.in_sideboard} showImagePreview={hoverAction} dropImagePreview={cancelHoverAction} />
                      ))}</div>
                    <div className="deckeditor__sideslotlands-container">
            ...
            {listFlag && (
              <div className="deckeditor__list-view">This is the list view</div>
            )}
            {stacksFlag && (
              <div className="deckeditor__stack-view">This is the stack view</div>
            )}
          </div>
          <div className="deckeditor__card-display">
            <div className="deckeditor__card-display-sticky-track">
              <div className="deckeditor__card-display-image" style={{ backgroundImage: `url(${imagePreview})` }}></div>
              {multiface.length > 0 && <div className="deckeditor__card-display-multiface-image" style={{ backgroundImage: `url(${imagePreview})` }}></div>}
            </div>
          </div>
        </div>
      </div>
